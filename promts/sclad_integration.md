...
## Сопоставление товаров подбор принципа.

Смотри в файле public/moysklad_assortment.json есть поле с массивом "rows" у каждого элемента есть поле "name". Я хочу в автоматическом режиме сопоставить данные из этого массива с элементами модели Product.

Проблемма в следующем у Product есть 'title' у в файле есть "rows"."name" они не соотвестствуют ( может быть разный регистр может быть различие в знаказ - колличестве пробелоы и т. д.).

## Я хочу подобрать алгоритмический метод сравнения который даст мне коэфициент похожести этих строк и на основе этого коэфициента я бы принимал решение о сопоставлении информации.php

Давай на основе твоих рекомендаций реализуем комманду которая выберет все элементы модели Product сопоставил их с данными из файла public/moysklad_assortment.json и если проходит по порогу перенесет в соответствующие поля модели Product данные из полей:

- "code"
- "externalCode"

фалйла moysklad_assortment.json

Давай добавим в созданную комманду команду возможность 'сухой' загрузки в этом режиме мы не будем устаноавливать значения для asc_nal а просто выведем результаты в консоль.

## Изменения в админку

В проект встроена админка https://orchid.software/en/discussions/

Твоя задача внести следующие изменения:

В файле app/Orchid/Layouts/Product/ProductListTable.php добавь в таблицу поле code с наименованием столбца Код Мой склад. Добавь поле skladCount с наименованием столбца На складе. Убери поле description.

В файлы

- app/Orchid/Screens/Product/ProductCreateScreen.php
- app/Orchid/Layouts/Product/ProductListTable.php

Добавь поля skladCount, code, externalCode
